...
void Game::Update(DX::StepTimer const& timer)
{
    ...
    std::cout << dataPtr->time << std::endl;
...
void Game::Render()
{
    ...
    //context->Map(m_chromAbberationParam.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
   
    ////D3D11_SUBRESOURCE_DATA initData = { &timeHandle, 0, 0 };

    //memcpy(resource.pData, init, sizeof(m_chromAbberationParam));
    //
    //context->Unmap(m_chromAbberationParam.Get(), 0);
...
void Game::CreateDeviceDependentResources()
{
    ...
    DX::ThrowIfFailed(device->CreatePixelShader(blob.data(), blob.size(),
        nullptr, m_blackWhitePS.ReleaseAndGetAddressOf()));*/
    
    
    auto blob = DX::ReadData(L"ChromAbberation.cso");
    DX::ThrowIfFailed(device->CreatePixelShader(blob.data(), blob.size(),
        nullptr, m_chromAbberation.ReleaseAndGetAddressOf()));

    
    /*CD3D11_BUFFER_DESC cbDesc(sizeof(PP_CHROMATIC_ABBERATION_PARAMETERS),
    ...
    device->CreateBuffer(&cbDesc, &initData,&constBuffer);
   ...
   /* for (auto& it : vertices)
    ...
    m_shape = GeometricPrimitive::CreateCustom(context, vertices, indices);
...
void Game::CreateWindowSizeDependentResources()
{
    ...
    //context->UpdateSubresource(m_blackWhiteParam.Get(), 0, nullptr, &bwData, sizeof(BWparams),0);
...
void Game::OnDeviceLost()
{
    ...
    m_blackWhitePS.Reset();
    m_chromAbberation.Reset();

    m_bloomParams.Reset();
    m_blurParamsWidth.Reset();
    m_blurParamsHeight.Reset();
    m_chromAbberationParam.Reset();
